...
import { X, Upload, Music, Image as ImageIcon, Loader2, AlertCircle, CheckCircle2, ChevronRight, Hash, Activity } from 'lucide-react';
...
export function AdminModal({ isOpen, onClose }: AdminModalProps) {
    const { isUploading, progress, status, error, uploadProject, resetState } = useAdminUpload();
    ...
    const handleHandleUpload = async (e: React.FormEvent) => {
        ...
        if (success) {
            setIsSuccess(true);
            // Reset after 3s or when closing
        }
    };
            ...
            <div className="bg-[#1c1c1e] w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col border border-white/10 my-auto animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between p-4 border-b border-white/10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-white font-bold text-lg">Painel Admin: Cloud Upload</h2>
                            <p className="text-xs text-text-muted">Publique músicas na biblioteca online</p>
                        </div>
                    </div>
                ...
                {isSuccess ? (
                    <div className="p-8 flex flex-col items-center justify-center text-center gap-4 animate-in fade-in slide-in-from-bottom-4">
                        ...
                        <h3 className="text-xl font-bold text-white">Música Publicada!</h3>
                        <p className="text-text-muted max-w-xs">Sua música já está disponível na biblioteca para download.</p>
                        <button onClick={handleClose} className="mt-4 px-8 py-3 bg-primary text-black font-bold rounded-xl hover:scale-105 active:scale-95 transition-all">
                            Fechar Painel
                        </button>
                    </div>
                ) : (
                    <form onSubmit={handleHandleUpload} className="p-4 sm:p-6 space-y-6">
                        ...
                        <button
                            type="submit"
                            disabled={isUploading || !metadata.name || stemFiles.length === 0}
                            className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-[0.98] ${isUploading
                                    ? 'bg-white/5 text-text-muted cursor-not-allowed'
                                    : 'bg-secondary text-black hover:shadow-[0_0_20px_rgba(5,209,255,0.3)]'
                                }`}>
                            {isUploading ? (
                                <>Processando...</>
                            ) : (
