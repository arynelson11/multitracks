...
// Barra fixa no rodapé que aparece quando há uma nova versão do app desktop.
...
type UpdateState =
  | { kind: 'none' }
  | { kind: 'ready'; version: string }       // Windows: baixado, pronto pra reiniciar
  | { kind: 'available'; version: string };   // Mac: existe versão nova pra baixar
...
export function UpdateBanner() {
  ...
  const handleAction = () => window.playbackDesktop?.installUpdate();
    ...
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[9999] w-[min(92vw,440px)]">
      <div className="flex items-center gap-3 bg-card border border-primary/30 rounded-xl shadow-2xl shadow-black/40 px-4 py-3">
        <div className="w-9 h-9 rounded-lg bg-primary/15 border border-primary/25 flex items-center justify-center shrink-0">
          {isReady ? <RefreshCw size={16} className="text-primary" /> : <Download size={16} className="text-primary" />}
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-white font-bold text-sm leading-tight">
            {isReady ? 'Atualização pronta' : 'Nova versão disponível'}
          </p>
